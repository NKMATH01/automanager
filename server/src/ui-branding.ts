const FAVICON_BLOCK_START = "<!-- AUTOMANAGER_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- AUTOMANAGER_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- AUTOMANAGER_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- AUTOMANAGER_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.AUTOMANAGER_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.AUTOMANAGER_WORKTREE_NAME) ?? nonEmpty(env.AUTOMANAGER_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.AUTOMANAGER_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="automanager-worktree-enabled" content="true" />',
    `<meta name="automanager-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="automanager-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="automanager-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
