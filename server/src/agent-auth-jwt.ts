...
function jwtConfig() {
  const secret = process.env.AUTOMANAGER_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.AUTOMANAGER_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.AUTOMANAGER_AGENT_JWT_ISSUER ?? "paperclip",
    audience: process.env.AUTOMANAGER_AGENT_JWT_AUDIENCE ?? "paperclip-api",
  };
}
