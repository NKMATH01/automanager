...
function resolveAutomanagerSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const paperclipSkillsDir = resolveAutomanagerSkillsDir();
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm automanager allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/paperclip-claimed-api-key.json and load AUTOMANAGER_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Paperclip hostname is reachable and allowed via `pnpm automanager allowed-hostname <host>`."
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    And set the AUTOMANAGER_API_KEY and AUTOMANAGER_API_URL in your environment variables as specified here:
    ...
    Then set AUTOMANAGER_API_KEY and AUTOMANAGER_API_URL from the saved token field for every heartbeat run.
    ...
    Be sure to prepend your AUTOMANAGER_API_URL to the top of your skill and note the path to your AUTOMANAGER_API_URL
  ...
  if (connectionCandidates.length > 0) {
    ...
    appendBlock(`
      ...
      - pnpm automanager allowed-hostname <host>
