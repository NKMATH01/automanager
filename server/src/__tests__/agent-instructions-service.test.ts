...
describe("agent instructions service", () => {
  const originalPaperclipHome = process.env.AUTOMANAGER_HOME;
  const originalPaperclipInstanceId = process.env.AUTOMANAGER_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalPaperclipHome === undefined) delete process.env.AUTOMANAGER_HOME;
    else process.env.AUTOMANAGER_HOME = originalPaperclipHome;
    if (originalPaperclipInstanceId === undefined) delete process.env.AUTOMANAGER_INSTANCE_ID;
    else process.env.AUTOMANAGER_INSTANCE_ID = originalPaperclipInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    ...
    process.env.AUTOMANAGER_INSTANCE_ID = "test-instance";
