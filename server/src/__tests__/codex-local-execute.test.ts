...
import { execute } from "@automanager/adapter-codex-local/server";

async function writeFakeCodexCommand(commandPath: string): Promise<void> {
  const script = `#!/usr/bin/env node
const fs = require("node:fs");

const capturePath = process.env.AUTOMANAGER_TEST_CAPTURE_PATH;
const payload = {
...
describe("codex execute", () => {
  it("uses a Paperclip-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    ...
    const previousPaperclipHome = process.env.AUTOMANAGER_HOME;
    const previousPaperclipInstanceId = process.env.AUTOMANAGER_INSTANCE_ID;
    const previousPaperclipInWorktree = process.env.AUTOMANAGER_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.AUTOMANAGER_HOME = paperclipHome;
    delete process.env.AUTOMANAGER_INSTANCE_ID;
    delete process.env.AUTOMANAGER_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            AUTOMANAGER_TEST_CAPTURE_PATH: capturePath,
          },
      ...
      if (previousPaperclipHome === undefined) delete process.env.AUTOMANAGER_HOME;
      else process.env.AUTOMANAGER_HOME = previousPaperclipHome;
      if (previousPaperclipInstanceId === undefined) delete process.env.AUTOMANAGER_INSTANCE_ID;
      else process.env.AUTOMANAGER_INSTANCE_ID = previousPaperclipInstanceId;
      if (previousPaperclipInWorktree === undefined) delete process.env.AUTOMANAGER_IN_WORKTREE;
      else process.env.AUTOMANAGER_IN_WORKTREE = previousPaperclipInWorktree;
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    ...
    try {
      ...
      expect(loggedEnv.AUTOMANAGER_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    ...
    process.env.HOME = root;
    process.env.AUTOMANAGER_HOME = paperclipHome;
    process.env.AUTOMANAGER_INSTANCE_ID = "worktree-1";
