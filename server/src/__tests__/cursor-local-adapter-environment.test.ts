...
import { testEnvironment } from "@automanager/adapter-cursor-local/server";
...
const outPath = process.env.AUTOMANAGER_TEST_ARGS_PATH;
if (outPath) {
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          AUTOMANAGER_TEST_ARGS_PATH: argsCapturePath,
