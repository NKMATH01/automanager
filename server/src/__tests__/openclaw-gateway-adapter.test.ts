...
import { execute, testEnvironment } from "@automanager/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@automanager/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(String(payload?.message ?? "")).toContain("AUTOMANAGER_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("AUTOMANAGER_TASK_ID=task-123");
