...
import { resolveAutomanagerConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@automanager/shared";
...
afterEach(async () => {
  ...
  delete process.env.AUTOMANAGER_CONFIG;
  delete process.env.AUTOMANAGER_HOME;
  delete process.env.AUTOMANAGER_INSTANCE_ID;
  delete process.env.AUTOMANAGER_WORKTREES_DIR;
...
describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Paperclip and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      ...
      AUTOMANAGER_HOME: "/tmp/paperclip-home",
      AUTOMANAGER_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.AUTOMANAGER_HOME).toBeUndefined();
    expect(sanitized.AUTOMANAGER_INSTANCE_ID).toBeUndefined();
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".automanager", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$AUTOMANAGER_WORKSPACE_BRANCH\" > .automanager-provision-branch",
        "printf '%s\\n' \"$AUTOMANAGER_WORKSPACE_BASE_CWD\" > .automanager-provision-base",
        "printf '%s\\n' \"$AUTOMANAGER_WORKSPACE_CREATED\" > .automanager-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".automanager-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".automanager-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".automanager-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".automanager-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("writes an isolated repo-local Paperclip config and worktree branding when provisioning", async () => {
    ...
    process.env.AUTOMANAGER_HOME = paperclipHome;
    process.env.AUTOMANAGER_INSTANCE_ID = instanceId;
    process.env.AUTOMANAGER_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".automanager", "config.json");
      const envPath = path.join(workspace.cwd, ".automanager", ".env");
      ...
      expect(envContents).toContain(`AUTOMANAGER_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`AUTOMANAGER_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`AUTOMANAGER_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("AUTOMANAGER_IN_WORKTREE=true");
      expect(envContents).toContain(
        `AUTOMANAGER_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveAutomanagerConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    ...
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".automanager", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.AUTOMANAGER_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              AUTOMANAGER_HOME: "{{workspace.cwd}}/.automanager/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".automanager", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".automanager", "runtime-services"));
  });
  ...
  it("does not leak parent Paperclip instance env into runtime service commands", async () => {
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "paperclipConfig: process.env.AUTOMANAGER_CONFIG ?? null,",
          "paperclipHome: process.env.AUTOMANAGER_HOME ?? null,",
          "paperclipInstanceId: process.env.AUTOMANAGER_INSTANCE_ID ?? null,",
    ...
    process.env.AUTOMANAGER_CONFIG = "/tmp/base-paperclip-config.json";
    process.env.AUTOMANAGER_HOME = "/tmp/base-paperclip-home";
    process.env.AUTOMANAGER_INSTANCE_ID = "base-instance";
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    ...
    process.env.AUTOMANAGER_HOME = paperclipHome;
    process.env.AUTOMANAGER_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
