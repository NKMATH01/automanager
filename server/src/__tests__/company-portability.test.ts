...
import type { CompanyPortabilityFileEntry } from "@automanager/shared";
...
describe("company portability", () => {
  const paperclipKey = "automanager/paperclip/paperclip";
    ...
    const companySkills = [
      {
        ...
        markdown: "---\nname: paperclip\ndescription: Paperclip coordination skill\n---\n\n# Paperclip\n",
        sourceType: "github",
        sourceLocator: "https://github.com/automanager/paperclip/tree/master/skills/paperclip",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/automanager/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "automanager",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/automanager/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "automanager",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Paperclip extension data", async () => {
    ...
    expect(asTextFile(exported.files["skills/automanager/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/automanager/paperclip/paperclip/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/automanager/paperclip/paperclip/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".automanager.yaml"]);
  ...
  it("exports default sidebar order into the Paperclip extension and manifest", async () => {
    ...
    expect(asTextFile(exported.files[".automanager.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/automanager/paperclip/paperclip/SKILL.md"])).toContain("# Paperclip");
    expect(asTextFile(exported.files["skills/automanager/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/automanager/paperclip/paperclip/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/automanager/paperclip/paperclip/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/automanager/paperclip/paperclip/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .automanager.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".automanager.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-paperclip",
        companyId: "company-1",
        key: "automanager/paperclip/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/automanager/paperclip/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/automanager/paperclip/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/automanager/paperclip/release-changelog/SKILL.md"])).toContain("automanager/paperclip/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            repoUrl: "https://github.com/automanager/paperclip.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .automanager-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/automanager/paperclip.git"');
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "https://github.com/automanager/paperclip.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
  ...
  it("reads env inputs back from .automanager.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports a vendor-neutral package without .automanager.yaml", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".automanager.yaml"] = `${exported.files[".automanager.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
