...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    ...
    const paperclipDir = path.join(worktreeRoot, ".automanager");
    const configPath = path.join(paperclipDir, "config.json");
    const envPath = path.join(paperclipDir, ".env");
    const sharedRoot = path.join(tempRoot, ".automanager", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".automanager-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        ...
        "AUTOMANAGER_WORKTREE_NAME=PAP-884-ai-commits-component",
        "AUTOMANAGER_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.AUTOMANAGER_IN_WORKTREE = "true";
    process.env.AUTOMANAGER_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.AUTOMANAGER_WORKTREES_DIR = isolatedHome;
    delete process.env.AUTOMANAGER_HOME;
    delete process.env.AUTOMANAGER_INSTANCE_ID;
    delete process.env.AUTOMANAGER_CONFIG;
    delete process.env.AUTOMANAGER_CONTEXT;
    ...
    expect(repairedEnv).toContain(`AUTOMANAGER_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('AUTOMANAGER_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`AUTOMANAGER_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`AUTOMANAGER_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('AUTOMANAGER_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.AUTOMANAGER_HOME).toBe(isolatedHome);
    expect(process.env.AUTOMANAGER_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });
  ...
  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "# Paperclip environment variables",
        "AUTOMANAGER_IN_WORKTREE=true",
        "AUTOMANAGER_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.AUTOMANAGER_IN_WORKTREE = "true";
    process.env.AUTOMANAGER_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.AUTOMANAGER_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    ...
    const isolatedHome = path.join(tempRoot, ".automanager-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".automanager", "worktrees");
    ...
    const paperclipDir = path.join(currentWorktreeRoot, ".automanager");
    const configPath = path.join(paperclipDir, "config.json");
    const envPath = path.join(paperclipDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".automanager", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    ...
    process.chdir(worktreeRoot);
    process.env.AUTOMANAGER_IN_WORKTREE = "true";
    process.env.AUTOMANAGER_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.AUTOMANAGER_HOME = isolatedHome;
    process.env.AUTOMANAGER_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.AUTOMANAGER_CONFIG = configPath;
