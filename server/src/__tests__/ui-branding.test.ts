...
<head>
    <!-- AUTOMANAGER_RUNTIME_BRANDING_START -->
    <!-- AUTOMANAGER_RUNTIME_BRANDING_END -->
    <!-- AUTOMANAGER_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- AUTOMANAGER_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from AUTOMANAGER_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ AUTOMANAGER_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ AUTOMANAGER_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ AUTOMANAGER_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      AUTOMANAGER_IN_WORKTREE: "true",
      AUTOMANAGER_WORKTREE_NAME: "paperclip-pr-432",
      AUTOMANAGER_WORKTREE_COLOR: "#4f86f7",
    });
