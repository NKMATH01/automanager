...
import type { AdapterRuntimeServiceReport } from "@automanager/adapter-utils";
import type { Db } from "@automanager/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@automanager/db";
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith("AUTOMANAGER_")) {
      delete env[key];
    }
  }
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AUTOMANAGER_WORKSPACE_CWD = input.worktreePath;
  env.AUTOMANAGER_WORKSPACE_PATH = input.worktreePath;
  env.AUTOMANAGER_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AUTOMANAGER_WORKSPACE_BRANCH = input.branchName;
  env.AUTOMANAGER_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AUTOMANAGER_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AUTOMANAGER_WORKSPACE_SOURCE = input.base.source;
  env.AUTOMANAGER_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AUTOMANAGER_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AUTOMANAGER_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AUTOMANAGER_PROJECT_ID = input.base.projectId ?? "";
  env.AUTOMANAGER_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AUTOMANAGER_AGENT_ID = input.agent.id ?? "";
  env.AUTOMANAGER_AGENT_NAME = input.agent.name;
  env.AUTOMANAGER_COMPANY_ID = input.agent.companyId;
  env.AUTOMANAGER_ISSUE_ID = input.issue?.id ?? "";
  env.AUTOMANAGER_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AUTOMANAGER_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AUTOMANAGER_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AUTOMANAGER_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AUTOMANAGER_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AUTOMANAGER_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AUTOMANAGER_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AUTOMANAGER_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AUTOMANAGER_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AUTOMANAGER_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AUTOMANAGER_PROJECT_ID = input.workspace.projectId ?? "";
  env.AUTOMANAGER_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AUTOMANAGER_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".automanager", "worktrees");
