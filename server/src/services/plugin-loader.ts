 ...
 *    (`~/.automanager/plugins/`) and `node_modules` for packages matching
...
export type PluginSource =
  | "local-filesystem"  // ~/.automanager/plugins/ local directory
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.automanager/plugins/
   */
...
export function isPluginPackageName(name: string): boolean {
  ...
  // Also accept scoped packages like @acme/plugin-linear or @automanager/plugin-*
  if (name.includes("/")) {
...
export function pluginLoader(
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @automanager/shared exports). Run those workers through
