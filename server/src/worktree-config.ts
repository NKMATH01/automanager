...
import type { PaperclipConfig } from "@automanager/shared";
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.AUTOMANAGER_IN_WORKTREE !== "true") return null;

  const configPath = resolveAutomanagerConfigPath(overrideConfigPath);
  const envPath = resolveAutomanagerEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.AUTOMANAGER_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.AUTOMANAGER_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.AUTOMANAGER_HOME) ??
      nonEmpty(env.AUTOMANAGER_WORKTREES_DIR) ??
      "~/.automanager-worktrees",
  );
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".automanager", "config.json");
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.AUTOMANAGER_HOME = context.homeDir;
  process.env.AUTOMANAGER_INSTANCE_ID = context.instanceId;
  process.env.AUTOMANAGER_CONFIG = context.configPath;
  process.env.AUTOMANAGER_CONTEXT = context.contextPath;
  process.env.AUTOMANAGER_WORKTREE_NAME = context.worktreeName;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    AUTOMANAGER_HOME: context.homeDir,
    AUTOMANAGER_INSTANCE_ID: context.instanceId,
    AUTOMANAGER_CONFIG: context.configPath,
    AUTOMANAGER_CONTEXT: context.contextPath,
    AUTOMANAGER_IN_WORKTREE: "true",
    AUTOMANAGER_WORKTREE_NAME: context.worktreeName,
  };
