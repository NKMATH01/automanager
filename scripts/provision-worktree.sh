...
base_cwd="${AUTOMANAGER_WORKSPACE_BASE_CWD:?AUTOMANAGER_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${AUTOMANAGER_WORKSPACE_CWD:?AUTOMANAGER_WORKSPACE_CWD is required}"
paperclip_home="${AUTOMANAGER_HOME:-$HOME/.automanager}"
paperclip_instance_id="${AUTOMANAGER_INSTANCE_ID:-default}"
paperclip_dir="$worktree_cwd/.automanager"
worktree_config_path="$paperclip_dir/config.json"
worktree_env_path="$paperclip_dir/.env"
worktree_name="${AUTOMANAGER_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${AUTOMANAGER_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.automanager/config.json" || -L "$base_cwd/.automanager/config.json" ) ]]; then
  source_config_path="$base_cwd/.automanager/config.json"
fi
...
run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm automanager --help >/dev/null 2>&1; then
    pnpm automanager worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v automanager >/dev/null 2>&1; then
    automanager worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  AUTOMANAGER_DIR="$paperclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  AUTOMANAGER_WORKTREES_DIR="${AUTOMANAGER_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const paperclipDir = process.env.AUTOMANAGER_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.AUTOMANAGER_WORKTREES_DIR) ?? "~/.automanager-worktrees"));
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.AUTOMANAGER_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.AUTOMANAGER_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.AUTOMANAGER_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "AUTOMANAGER_HOME=" + JSON.stringify(worktreeHome),
    "AUTOMANAGER_INSTANCE_ID=" + JSON.stringify(instanceId),
    "AUTOMANAGER_CONFIG=" + JSON.stringify(configPath),
    "AUTOMANAGER_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "AUTOMANAGER_IN_WORKTREE=true",
    "AUTOMANAGER_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.AUTOMANAGER_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("AUTOMANAGER_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "automanager CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
