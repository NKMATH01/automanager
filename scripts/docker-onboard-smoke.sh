...
AUTOMANAGER_DEPLOYMENT_MODE="${AUTOMANAGER_DEPLOYMENT_MODE:-authenticated}"
AUTOMANAGER_DEPLOYMENT_EXPOSURE="${AUTOMANAGER_DEPLOYMENT_EXPOSURE:-private}"
AUTOMANAGER_PUBLIC_URL="${AUTOMANAGER_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e AUTOMANAGER_DEPLOYMENT_MODE="$AUTOMANAGER_DEPLOYMENT_MODE" \
      -e AUTOMANAGER_DEPLOYMENT_EXPOSURE="$AUTOMANAGER_DEPLOYMENT_EXPOSURE" \
      -e AUTOMANAGER_PUBLIC_URL="$AUTOMANAGER_PUBLIC_URL" \
      -e AUTOMANAGER_HOME="/paperclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "automanager@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$AUTOMANAGER_HOME" --base-url "$AUTOMANAGER_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $AUTOMANAGER_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$AUTOMANAGER_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$AUTOMANAGER_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$AUTOMANAGER_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$AUTOMANAGER_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$AUTOMANAGER_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$AUTOMANAGER_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $AUTOMANAGER_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $AUTOMANAGER_DEPLOYMENT_MODE/$AUTOMANAGER_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$AUTOMANAGER_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $AUTOMANAGER_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$AUTOMANAGER_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $AUTOMANAGER_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
