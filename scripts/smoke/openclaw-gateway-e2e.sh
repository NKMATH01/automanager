...
AUTOMANAGER_API_URL="${AUTOMANAGER_API_URL:-http://127.0.0.1:3100}"
API_BASE="${AUTOMANAGER_API_URL%/}/api"
...
AUTOMANAGER_API_URL_FOR_OPENCLAW="${AUTOMANAGER_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${AUTOMANAGER_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${AUTOMANAGER_AUTH_HEADER}" )
fi
if [[ -n "${AUTOMANAGER_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${AUTOMANAGER_COOKIE}" )
  AUTOMANAGER_BROWSER_ORIGIN="${AUTOMANAGER_BROWSER_ORIGIN:-${AUTOMANAGER_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${AUTOMANAGER_BROWSER_ORIGIN}" -H "Referer: ${AUTOMANAGER_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set AUTOMANAGER_COOKIE or AUTOMANAGER_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg paperclipApiUrl "$AUTOMANAGER_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    ...
    assert_status "200"
    {
      echo "AUTOMANAGER_API_URL=${AUTOMANAGER_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set AUTOMANAGER_API_KEY=${AGENT_API_KEY} in your run context before running Paperclip heartbeat steps."
...
main() {
  ...
  wait_http_ready "${AUTOMANAGER_API_URL%/}/api/health" 15 || fail "Paperclip API health endpoint not reachable"
