...
COMPANY_ID="${COMPANY_ID:-${AUTOMANAGER_COMPANY_ID:-}}"
...
Provide one of:
  AUTOMANAGER_AUTH_HEADER=\"Bearer <board-token>\"
  AUTOMANAGER_COOKIE=\"<board-session-cookie>\"
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.automanager.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
