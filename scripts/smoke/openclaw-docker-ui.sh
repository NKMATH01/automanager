...
AUTOMANAGER_HOST_PORT="${AUTOMANAGER_HOST_PORT:-3100}"
AUTOMANAGER_HOST_FROM_CONTAINER="${AUTOMANAGER_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.automanager-openclaw.override.yml"
...
detect_paperclip_base_url() {
  ...
  for candidate in "$AUTOMANAGER_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${AUTOMANAGER_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${AUTOMANAGER_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${AUTOMANAGER_HOST_PORT}
  (Do not use http://127.0.0.1:${AUTOMANAGER_HOST_PORT} inside the container.)
  If Paperclip rejects the host, run on host machine:
    pnpm automanager allowed-hostname host.docker.internal
