...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.automanager/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.automanager/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.automanager/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/paperclip*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.automanager/instances/*/db/postmaster.pid
    "$sibling_root"/.automanager/runtime-services/instances/*/db/postmaster.pid
  )
done
