...
const devServerStatusFilePath = path.join(repoRoot, ".automanager", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".automanager/dev-server-status.json",
]);
...
const env: NodeJS.ProcessEnv = {
  ...process.env,
  AUTOMANAGER_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.AUTOMANAGER_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.AUTOMANAGER_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.AUTOMANAGER_MIGRATION_PROMPT ??= "never";
  env.AUTOMANAGER_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.AUTOMANAGER_DEPLOYMENT_MODE = "authenticated";
  env.AUTOMANAGER_DEPLOYMENT_EXPOSURE = "private";
  env.AUTOMANAGER_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@automanager/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclip] Command failed with code ${status.code}: pnpm --filter @automanager/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.AUTOMANAGER_MIGRATION_AUTO_APPLY === "true";
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@automanager/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@automanager/server", serverScript, ...forwardedArgs],
