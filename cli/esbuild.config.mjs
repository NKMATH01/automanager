/**
 * esbuild configuration for building the automanager CLI for npm.
 *
 * Bundles all workspace packages (@automanager/*) into a single file.
...
// to npm and resolved at runtime (e.g. @automanager/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@automanager/")) {
      externals.add(name);
    }
  }
