...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "오토매니저 API에 연결할 수 없습니다.",
    "",
    `요청: ${input.method} ${input.url}`,
  ];
  if (input.causeMessage) {
    lines.push(`원인: ${input.causeMessage}`);
  }
  lines.push(
    "",
    "이는 보통 오토매니저 서버가 실행 중이지 않거나, 설정된 URL이 잘못되었거나, 요청이 서버에 도달하기 전에 차단되고 있음을 의미합니다.",
    "",
    "다음을 시도하세요:",
    "- `pnpm dev` 또는 `pnpm automanager run`으로 오토매니저를 시작하세요.",
    `- \`curl ${healthUrl}\`로 서버 접근 가능 여부를 확인하세요.`,
    `- 오토매니저가 다른 곳에서 실행 중이라면 \`--api-base ${input.apiBase.replace(/\/+$/, "")}\`를 전달하거나 \`AUTOMANAGER_API_URL\`을 설정하세요.`,
  );
  return lines.join("\n");
}
