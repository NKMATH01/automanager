...
function withStrictModeNote(
  ...
  return {
    ...base,
    status: "warn",
    message: `${base.message}; postgres 배포에서 엄격 시크릿 모드가 비활성화되어 있습니다`,
...
export function secretsCheck(config: PaperclipConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      name: "시크릿 어댑터",
      status: "fail",
      message: `${provider}이(가) 설정되었지만 이 빌드는 local_encrypted만 지원합니다`,
      canRepair: false,
      repairHint: "`automanager configure --section secrets`를 실행하고 제공자를 local_encrypted로 설정하세요",
    };
  }

  const envMasterKey = process.env.AUTOMANAGER_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "AUTOMANAGER_SECRETS_MASTER_KEY가 유효하지 않습니다 (32바이트 base64, 64자 hex 또는 32자 원시 문자열이어야 합니다)",
        canRepair: false,
        repairHint: "AUTOMANAGER_SECRETS_MASTER_KEY를 유효한 키로 설정하거나 키 파일을 사용하려면 설정을 해제하세요",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "시크릿 어댑터",
        status: "pass",
        message: "AUTOMANAGER_SECRETS_MASTER_KEY를 통해 로컬 암호화 제공자가 설정됨",
      },
      config,
    );
  }

  const keyFileOverride = process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "파일 권한을 확인하거나 AUTOMANAGER_SECRETS_MASTER_KEY를 설정하세요",
    };
  }
  ...
  if (!decodeMasterKey(raw)) {
    return {
      name: "시크릿 어댑터",
      status: "fail",
      message: `${keyFilePath}에 유효하지 않은 키 자료`,
      canRepair: false,
  ...
  return withStrictModeNote(
    {
      name: "시크릿 어댑터",
      status: "pass",
      message: `키 파일 ${keyFilePath}로 로컬 암호화 제공자가 설정됨`,
    },
    config,
  );
}
