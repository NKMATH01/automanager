...
export async function llmCheck(config: PaperclipConfig): Promise<CheckResult> {
  ...
  if (!config.llm.apiKey) {
    return {
      name: "LLM 제공자",
      status: "pass",
      message: `${config.llm.provider} 설정됨, API 키 미설정 (선택사항)`,
    };
  }
    ...
    if (config.llm.provider === "claude") {
      ...
      if (res.status === 401) {
        return {
          ...
          canRepair: false,
          repairHint: "`automanager configure --section llm`을 실행하세요",
        };
      }
      return {
        name: "LLM 제공자",
        status: "warn",
        message: `Claude API가 상태 ${res.status}를 반환했습니다`,
      };
    } else {
      ...
      return {
        name: "LLM 제공자",
        status: "warn",
        message: `OpenAI API가 상태 ${res.status}를 반환했습니다`,
      };
    }
  } catch {
    return {
      name: "LLM 제공자",
      status: "warn",
      message: "키 검증을 위해 API에 연결할 수 없습니다",
    };
  }
}
