...
export function storageCheck(config: PaperclipConfig, configPath?: string): CheckResult {
  if (config.storage.provider === "local_disk") {
    ...
    try {
      ...
      return {
        name: "스토리지",
        status: "fail",
        message: `로컬 스토리지 디렉터리에 쓰기 불가: ${baseDir}`,
        canRepair: false,
  ...
  if (!bucket || !region) {
    return {
      ...
      canRepair: false,
      repairHint: "`automanager configure --section storage`를 실행하세요",
    };
  }
  ...
  return {
    name: "스토리지",
    status: "warn",
    message: `S3 스토리지 설정됨 (bucket=${bucket}, region=${region}). 접근 가능성 검사는 doctor에서 생략됩니다.`,
    canRepair: false,
