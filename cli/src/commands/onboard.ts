...
const ONBOARD_ENV_KEYS = [
  "AUTOMANAGER_PUBLIC_URL",
  "DATABASE_URL",
  "AUTOMANAGER_DB_BACKUP_ENABLED",
  "AUTOMANAGER_DB_BACKUP_INTERVAL_MINUTES",
  "AUTOMANAGER_DB_BACKUP_RETENTION_DAYS",
  "AUTOMANAGER_DB_BACKUP_DIR",
  "AUTOMANAGER_DEPLOYMENT_MODE",
  "AUTOMANAGER_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "AUTOMANAGER_ALLOWED_HOSTNAMES",
  "AUTOMANAGER_AUTH_BASE_URL_MODE",
  "AUTOMANAGER_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "AUTOMANAGER_STORAGE_PROVIDER",
  "AUTOMANAGER_STORAGE_LOCAL_DIR",
  "AUTOMANAGER_STORAGE_S3_BUCKET",
  "AUTOMANAGER_STORAGE_S3_REGION",
  "AUTOMANAGER_STORAGE_S3_ENDPOINT",
  "AUTOMANAGER_STORAGE_S3_PREFIX",
  "AUTOMANAGER_STORAGE_S3_FORCE_PATH_STYLE",
  "AUTOMANAGER_SECRETS_PROVIDER",
  "AUTOMANAGER_SECRETS_STRICT_MODE",
  "AUTOMANAGER_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveAutomanagerInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.AUTOMANAGER_PUBLIC_URL?.trim() ||
    process.env.AUTOMANAGER_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.AUTOMANAGER_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.AUTOMANAGER_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.AUTOMANAGER_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.AUTOMANAGER_ALLOWED_HOSTNAMES
    ? process.env.AUTOMANAGER_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.AUTOMANAGER_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.AUTOMANAGER_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.AUTOMANAGER_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.AUTOMANAGER_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.AUTOMANAGER_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.AUTOMANAGER_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.AUTOMANAGER_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.AUTOMANAGER_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.AUTOMANAGER_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.AUTOMANAGER_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.AUTOMANAGER_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.AUTOMANAGER_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.AUTOMANAGER_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.AUTOMANAGER_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "AUTOMANAGER_DEPLOYMENT_EXPOSURE",
      reason: "배포 모드 local_trusted는 항상 비공개 노출을 강제하므로 무시됨",
    });
  }
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" automanager onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveAutomanagerInstanceId());
  p.log.message(
    pc.dim(
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("기존 오토매니저 설치가 감지되었습니다. 현재 설정을 유지합니다."),
    );
    p.log.message(pc.dim(`설정을 변경하려면 ${pc.cyan("automanager configure")}를 사용하세요.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`${pc.dim(envFilePath)}에 ${pc.cyan("AUTOMANAGER_AGENT_JWT_SECRET")} 생성됨`);
    } else if (process.env.AUTOMANAGER_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`환경에서 기존 ${pc.cyan("AUTOMANAGER_AGENT_JWT_SECRET")} 사용 중`);
    } else {
      p.log.info(`${pc.dim(envFilePath)}의 기존 ${pc.cyan("AUTOMANAGER_AGENT_JWT_SECRET")} 사용 중`);
    }

    const keyResult = ensureLocalSecretsKeyFile(existingConfig, configPath);
    if (keyResult.status === "created") {
      p.log.success(`로컬 시크릿 키 파일 생성됨: ${pc.dim(keyResult.path)}`);
    } else if (keyResult.status === "existing") {
      p.log.message(pc.dim(`기존 로컬 시크릿 키 파일 사용 중: ${keyResult.path}`));
    }

    p.note(
      [
        "기존 설정 유지됨",
        `데이터베이스: ${existingConfig.database.mode}`,
        ...
        `스토리지: ${existingConfig.storage.provider}`,
        `시크릿: ${existingConfig.secrets.provider} (엄격 모드 ${existingConfig.secrets.strictMode ? "켜짐" : "꺼짐"})`,
        "에이전트 인증: AUTOMANAGER_AGENT_JWT_SECRET 설정됨",
      ].join("\n"),
      "설정 준비 완료",
    );
    ...
    p.note(
      [
        `실행: ${pc.cyan("automanager run")}`,
        `나중에 재설정: ${pc.cyan("automanager configure")}`,
        `설정 진단: ${pc.cyan("automanager doctor")}`,
      ].join("\n"),
      "다음 명령어",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.AUTOMANAGER_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("기존 오토매니저 설정이 준비되었습니다.");
    return;
  }

  let setupMode: SetupMode = "quickstart";
  if (opts.yes) {
    p.log.message(pc.dim("`--yes` 활성화: 빠른 설정 기본값을 사용합니다."));
  } else {
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("데이터베이스에 연결할 수 없습니다 — 나중에 `automanager doctor`로 수정할 수 있습니다"));
      }
    }
    ...
    p.log.step(pc.bold("빠른 설정"));
    p.log.message(pc.dim("빠른 설정 기본값을 사용합니다."));
    if (usedEnvKeys.length > 0) {
      p.log.message(pc.dim(`환경 변수 기반 기본값 활성화 (${usedEnvKeys.length}개 환경 변수 감지됨).`));
    } else {
      p.log.message(
        pc.dim("환경 변수 재정의 없음: 내장 데이터베이스, 파일 스토리지, 로컬 암호화 시크릿."),
      );
    }
  ...
  p.note(
    [
      ...
      `스토리지: ${storage.provider}`,
      `시크릿: ${secrets.provider} (엄격 모드 ${secrets.strictMode ? "켜짐" : "꺼짐"})`,
      "에이전트 인증: AUTOMANAGER_AGENT_JWT_SECRET 설정됨",
    ].join("\n"),
    "설정 저장됨",
  );
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `다음: ${pc.cyan("automanager run")}`,
        `그 후: ${pc.cyan("automanager auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
