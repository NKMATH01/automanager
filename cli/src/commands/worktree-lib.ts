...
export const DEFAULT_WORKTREE_HOME = "~/.automanager-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".automanager");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeEnvEntries(
  ...
  return {
    AUTOMANAGER_HOME: paths.homeDir,
    AUTOMANAGER_INSTANCE_ID: paths.instanceId,
    AUTOMANAGER_CONFIG: paths.configPath,
    AUTOMANAGER_CONTEXT: paths.contextPath,
    AUTOMANAGER_IN_WORKTREE: "true",
    ...(branding?.name ? { AUTOMANAGER_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { AUTOMANAGER_WORKTREE_COLOR: branding.color } : {}),
  };
}
