...
import { ensureAgentJwtSecret, loadPaperclipEnvFile, mergePaperclipEnvEntries, readPaperclipEnvEntries, resolveAutomanagerEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.AUTOMANAGER_CONFIG;
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.AUTOMANAGER_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.AUTOMANAGER_WORKTREE_START_POINT) ?? undefined;
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.automanager${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".automanager", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".automanager", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.automanager"));
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.AUTOMANAGER_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AUTOMANAGER_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.AUTOMANAGER_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveAutomanagerEnvFile(input.sourceConfigPath);
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readPaperclipEnvEntries(resolveAutomanagerEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.AUTOMANAGER_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.AUTOMANAGER_AGENT_JWT_SECRET);
  mergePaperclipEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { AUTOMANAGER_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" automanager worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printPaperclipCliBanner();
  p.intro(pc.bgCyan(pc.black(" automanager worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasPaperclipConfig: existsSync(path.resolve(worktreePath, ".automanager", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printPaperclipCliBanner();
  p.intro(pc.bgCyan(pc.black(" automanager worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveAutomanagerEnvFile(configPath);
  const envEntries = readPaperclipEnvEntries(envPath);
  const out = {
    AUTOMANAGER_CONFIG: configPath,
    ...(envEntries.AUTOMANAGER_HOME ? { AUTOMANAGER_HOME: envEntries.AUTOMANAGER_HOME } : {}),
    ...(envEntries.AUTOMANAGER_INSTANCE_ID ? { AUTOMANAGER_INSTANCE_ID: envEntries.AUTOMANAGER_INSTANCE_ID } : {}),
    ...(envEntries.AUTOMANAGER_CONTEXT ? { AUTOMANAGER_CONTEXT: envEntries.AUTOMANAGER_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      ...
      .map((choice) => path.resolve(choice.worktree, ".automanager", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readPaperclipEnvEntries(resolveAutomanagerEnvFile(configPath));
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".automanager", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".automanager", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .automanager/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  if (choices.length === 0) {
    throw new Error("오토매니저 워크트리를 찾을 수 없습니다. `automanager worktree:list`를 실행하여 레포 워크트리를 확인하세요.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("소스와 대상 오토매니저 설정이 동일합니다. 다른 --from/--to 워크트리를 선택하세요.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("워크트리 로컬 오토매니저 인스턴스 도우미");

  program
    .command("worktree:make")
    .description("~/NAME을 git 워크트리로 생성한 후 그 안에 격리된 오토매니저 인스턴스 초기화")
    .argument("<name>", "Worktree name — auto-prefixed with paperclip- if needed (created at ~/paperclip-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: AUTOMANAGER_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: AUTOMANAGER_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source AUTOMANAGER_HOME used when deriving the source config")
  ...
  worktree
    .command("init")
    .description("이 워크트리를 위한 레포 로컬 설정/환경 및 격리된 인스턴스 생성")
  ...
  worktree
    .command("env")
    .description("현재 워크트리 로컬 오토매니저 인스턴스의 셸 export 출력")
  ...
  program
    ...
    .description("이 레포에서 보이는 git 워크트리 목록 및 오토매니저 워크트리 여부")
    .option("--json", "Print JSON instead of text output")
    .action(worktreeListCommand);

  program
    .command("worktree:merge-history")
    .description("다른 워크트리의 이슈/댓글 기록을 현재 인스턴스로 미리보기 또는 가져오기")
  ...
  program
    ...
    .description("워크트리, 브랜치 및 격리된 인스턴스 데이터를 안전하게 제거")
