...
import type { Agent, HeartbeatRun, HeartbeatRunEvent, HeartbeatRunStatus } from "@automanager/shared";
...
export async function heartbeatRun(opts: HeartbeatRunOptions): Promise<void> {
  ...
  if (!agent || typeof agent !== "object" || !agent.id) {
    console.error(pc.red(`에이전트를 찾을 수 없습니다: ${opts.agentId}`));
    return;
  }
  ...
  if (!invokeRes) {
    console.error(pc.red("하트비트 호출 실패"));
    return;
  }
  if ((invokeRes as { status?: string }).status === "skipped") {
    console.log(pc.yellow("하트비트 호출이 건너뛰어졌습니다"));
    return;
  }

  const run = invokeRes as HeartbeatRun;
  console.log(pc.cyan(`에이전트 ${agent.name} (${agent.id})에 대해 하트비트 실행 ${run.id} 호출됨`));
  ...
  if (!activeRunId) {
    console.error(pc.red("하트비트 실행 ID를 캡처하지 못했습니다"));
    return;
  }
  ...
  while (true) {
    ...
    if (!currentRun) {
      console.error(pc.red("하트비트 실행이 사라졌습니다"));
      break;
    }
    ...
    if (deadline && Date.now() >= deadline) {
      finalError = `CLI가 ${timeoutMs}ms 후 시간 초과되었습니다`;
  ...
  if (finalStatus) {
    ...
    const label = `실행 ${activeRunId}이(가) 상태 ${finalStatus}(으)로 완료됨`;
    ...
    if (finalError) {
      console.log(pc.red(`오류: ${finalError}`));
    }
    if (finalRun) {
      ...
      if (resultObj) {
        ...
        if (subtype || isError || errors.length > 0 || resultText) {
          console.log(pc.red("Claude 결과 상세:"));
    ...
    process.exitCode = 1;
    console.log(pc.gray("하트비트 스트림이 최종 상태 없이 종료되었습니다"));
  }
}
