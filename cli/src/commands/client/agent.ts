...
import type { Agent } from "@automanager/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveAutomanagerSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export AUTOMANAGER_API_URL='${escaped(input.apiBase)}'`,
    `export AUTOMANAGER_COMPANY_ID='${escaped(input.companyId)}'`,
    `export AUTOMANAGER_AGENT_ID='${escaped(input.agentId)}'`,
    `export AUTOMANAGER_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}

export function registerAgentCommands(program: Command): void {
  const agent = program.command("agent").description("에이전트 작업");

  addCommonClientOptions(
    agent
      .command("list")
      .description("회사의 에이전트 목록 조회")
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "에이전트 API 키 생성, Codex/Claude용 로컬 오토매니저 스킬 설치, 셸 export 출력",
      )
      ...
      .option(
        "--no-install-skills",
        "~/.codex/skills 및 ~/.claude/skills에 오토매니저 스킬 설치 건너뛰기",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveAutomanagerSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "로컬 오토매니저 스킬 디렉터리를 찾을 수 없습니다. 레포 체크아웃에 ./skills가 필요합니다.",
              );
            }
