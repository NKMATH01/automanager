...
export function registerIssueCommands(program: Command): void {
  const issue = program.command("issue").description("이슈 작업");

  addCommonClientOptions(
    issue
      .command("list")
      .description("회사의 이슈 목록 조회")
  ...
  addCommonClientOptions(
    issue
      .command("get")
      .description("UUID 또는 식별자로 이슈 조회 (예: PC-12)")
  ...
  addCommonClientOptions(
    issue
      .command("release")
      .description("이슈를 할일로 되돌리고 담당자 해제")
      .argument("<issueId>", "Issue ID")
