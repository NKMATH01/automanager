...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isPaperclipYaml = baseName === ".automanager.yaml" || baseName === ".automanager.yml";
...
function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".automanager.yaml"] !== undefined) return ".automanager.yaml";
  if (files[".automanager.yml"] !== undefined) return ".automanager.yml";
...
export function registerCompanyCommands(program: Command): void {
  const company = program.command("company").description("회사 작업");
  ...
  addCommonClientOptions(
    company
      .command("export")
      .description("회사를 이동 가능한 마크다운 패키지로 내보내기")
      .argument("<companyId>", "Company ID")
  ...
  addCommonClientOptions(
    company
      .command("import")
      .description("로컬 경로, URL 또는 GitHub에서 이동 가능한 마크다운 회사 패키지 가져오기")
  ...
  addCommonClientOptions(
    company
      .command("delete")
      .description("ID 또는 약칭/접두사로 회사 삭제 (파괴적)")
