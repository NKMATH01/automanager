...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "오토매니저 설정 파일 경로")
    ...
    .option("--api-base <url>", "오토매니저 API 기본 URL")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AUTOMANAGER_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.AUTOMANAGER_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.AUTOMANAGER_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AUTOMANAGER_COMPANY_ID, or set context profile companyId via `automanager context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AUTOMANAGER_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AUTOMANAGER_SERVER_PORT || "");
