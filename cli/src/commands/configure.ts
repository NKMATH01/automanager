...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" automanager configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("설정 파일을 찾을 수 없습니다. 먼저 `automanager onboard`를 실행하세요.");
    p.outro("");
    return;
  }
  ...
  try {
    ...
    p.log.message(
      pc.yellow(
        `기존 설정이 유효하지 않습니다. 지금 복구할 수 있도록 기본값을 불러옵니다.\n${err instanceof Error ? err.message : String(err)}`,
      ),
    );
  ...
  if (section && !SECTION_LABELS[section]) {
    p.log.error(`알 수 없는 섹션: ${section}. 다음 중 선택하세요: ${Object.keys(SECTION_LABELS).join(", ")}`);
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("로컬이 아닌 제공자이므로 로컬 키 파일 관리를 건너뜁니다"));
          } else {
            p.log.message(pc.dim("AUTOMANAGER_SECRETS_MASTER_KEY가 설정되어 있어 로컬 키 파일 관리를 건너뜁니다"));
          }
        }
        break;
    }
