...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" automanager doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "`automanager configure --section database` 또는 `automanager onboard`를 실행하세요",
    };
...
async function maybeRepair(
  ...
  if (shouldRepair) {
    try {
      await result.repair();
      p.log.success(`복구됨: ${result.name}`);
      return true;
    } catch (err) {
      p.log.error(`복구 실패: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  return false;
}
