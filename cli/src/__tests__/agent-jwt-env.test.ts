...
describe("agent jwt env helpers", () => {
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("AUTOMANAGER_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "AUTOMANAGER_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.AUTOMANAGER_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "AUTOMANAGER_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
  ...
  it("quotes hash-prefixed env values so dotenv round-trips them", () => {
    ...
    mergePaperclipEnvEntries(
      {
        AUTOMANAGER_WORKTREE_COLOR: "#439edb",
      },
      envPath,
    );

    const contents = fs.readFileSync(envPath, "utf-8");
    expect(contents).toContain('AUTOMANAGER_WORKTREE_COLOR="#439edb"');
    expect(readPaperclipEnvEntries(envPath).AUTOMANAGER_WORKTREE_COLOR).toBe("#439edb");
  });
});
