...
describe("isGithubShorthand", () => {
  it("matches owner/repo/path shorthands", () => {
    expect(isGithubShorthand("automanager/companies/gstack")).toBe(true);
    expect(isGithubShorthand("automanager/companies")).toBe(true);
  });
...
describe("normalizeGithubImportSource", () => {
  it("normalizes shorthand imports to canonical GitHub sources", () => {
    expect(normalizeGithubImportSource("automanager/companies/gstack")).toBe(
      "https://github.com/automanager/companies?ref=main&path=gstack",
    );
  });

  it("applies --ref to shorthand imports", () => {
    expect(normalizeGithubImportSource("automanager/companies/gstack", "feature/demo")).toBe(
      "https://github.com/automanager/companies?ref=feature%2Fdemo&path=gstack",
    );
  });
  ...
  it("applies --ref to existing GitHub tree URLs without losing the package path", () => {
    expect(
      normalizeGithubImportSource(
        "https://github.com/automanager/companies/tree/main/gstack",
        "release/2026-03-23",
      ),
    ).toBe(
      "https://github.com/automanager/companies?ref=release%2F2026-03-23&path=gstack",
    );
  });
});
