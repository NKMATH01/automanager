...
describe("worktree helpers", () => {
  ...
  it("builds isolated config and env paths for a worktree", () => {
    ...
    expect(env.AUTOMANAGER_HOME).toBe(path.resolve("/tmp/paperclip-worktrees"));
    expect(env.AUTOMANAGER_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.AUTOMANAGER_IN_WORKTREE).toBe("true");
    expect(env.AUTOMANAGER_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.AUTOMANAGER_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export AUTOMANAGER_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    ...
    const originalInlineMasterKey = process.env.AUTOMANAGER_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.AUTOMANAGER_SECRETS_MASTER_KEY;
      delete process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.AUTOMANAGER_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.AUTOMANAGER_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          AUTOMANAGER_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    ...
    const originalJwtSecret = process.env.AUTOMANAGER_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.AUTOMANAGER_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".automanager-worktrees"),
      });

      const envPath = path.join(repoRoot, ".automanager", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("AUTOMANAGER_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("AUTOMANAGER_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/AUTOMANAGER_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.AUTOMANAGER_AGENT_JWT_SECRET;
      } else {
        process.env.AUTOMANAGER_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    ...
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".automanager-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".automanager", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Paperclip config", () => {
    ...
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".automanager", "config.json");
    const originalCwd = process.cwd();
    const originalPaperclipConfig = process.env.AUTOMANAGER_CONFIG;
    ...
    try {
      ...
      if (originalPaperclipConfig === undefined) {
        ...
        process.env.AUTOMANAGER_CONFIG = originalPaperclipConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(worktreePath, ".automanager", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".automanager", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
