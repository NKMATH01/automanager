...
import type { SecretProvider } from "@automanager/shared";
import type { SecretsConfig } from "../config/schema.js";
import { resolveDefaultSecretsKeyFilePath, resolveAutomanagerInstanceId } from "../config/home.js";
...
export async function promptSecrets(current?: SecretsConfig): Promise<SecretsConfig> {
  ...
  if (provider !== "local_encrypted") {
    p.note(
      `${provider}는 이 빌드에서 아직 완전히 연결되지 않았습니다. 해당 어댑터를 직접 구현하지 않는 한 local_encrypted를 유지하세요.`,
      "참고",
    );
  }
