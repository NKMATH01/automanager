...
export async function promptDatabase(current?: DatabaseConfig): Promise<DatabaseConfig> {
  ...
  if (mode === "postgres") {
    const value = await p.text({
      ...
      validate: (val) => {
        if (!val) return "PostgreSQL 모드에서는 연결 문자열이 필수입니다";
    ...
    const dataDir = await p.text({
      message: "내장 PostgreSQL 데이터 디렉터리",
  ...
  const backupDirInput = await p.text({
    ...
    validate: (val) => (!val || val.trim().length === 0 ? "백업 디렉터리는 필수입니다" : undefined),
  });
  ...
  const backupIntervalInput = await p.text({
    ...
    validate: (val) => {
      const n = Number(val);
      if (!Number.isInteger(n) || n < 1) return "주기는 양의 정수여야 합니다";
      if (n > 10080) return "주기는 10080분 (7일) 이하여야 합니다";
      return undefined;
    },
  });
