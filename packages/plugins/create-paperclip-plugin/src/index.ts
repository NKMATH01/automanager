...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".automanager-sdk");
  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@automanager/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@automanager/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@automanager/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import type { PaperclipPluginManifestV1 } from "@automanager/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@automanager/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@automanager/plugin-sdk/ui";
...
import { createTestHarness } from "@automanager/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@automanager/plugin-sdk\` and \`@automanager/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.automanager-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@automanager/plugin-sdk/bundlers\`.
