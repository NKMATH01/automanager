...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveAutomanagerHomeDir(), "instances", resolveAutomanagerInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".automanager", CONFIG_BASENAME);
...
function resolveAutomanagerConfigPath(): string {
  if (process.env.AUTOMANAGER_CONFIG?.trim()) {
    return path.resolve(process.env.AUTOMANAGER_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveAutomanagerEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveAutomanagerEnvPath(configPath);
