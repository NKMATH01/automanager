...
function resolveAutomanagerInstanceId(): string {
  const raw = process.env.AUTOMANAGER_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveAutomanagerHomeDir(), "instances", resolveAutomanagerInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveAutomanagerHomeDir(), "instances", resolveAutomanagerInstanceId(), "data", "backups");
}
