...
const AUTOMANAGER_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "AUTOMANAGER_RESOLVED_COMMAND"] = resolvedCommand;
  }
...
export function buildPaperclipEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    AUTOMANAGER_AGENT_ID: agent.id,
    AUTOMANAGER_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.AUTOMANAGER_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
...
export async function resolveAutomanagerSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...AUTOMANAGER_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listPaperclipSkillEntries(
  ...
  const root = await resolveAutomanagerSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `automanager/paperclip/${entry.name}`,
...
export function resolveAutomanagerDesiredSkillNames(
  config: Record<string, unknown>,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // within a Claude Code session (e.g. `npx automanager run` in a terminal
    ...
    void resolveSpawnTarget(command, args, opts.cwd, mergedEnv)
      .then((target) => {
        ...
        const decodeChunk = (decoder: StringDecoder, chunk: unknown) => {
        ...
        child.stdout?.on("data", (chunk: unknown) => {
          appendLog("stdout", decodeChunk(stdoutDecoder, chunk));
        });

        child.stderr?.on("data", (chunk: unknown) => {
          appendLog("stderr", decodeChunk(stderrDecoder, chunk));
        });
        ...
        child.on("close", (code: number | null, signal: NodeJS.Signals | null) => {
          if (timeout) clearTimeout(timeout);
          runningProcesses.delete(runId);
          appendLog("stdout", stdoutDecoder.end());
          appendLog("stderr", stderrDecoder.end());
