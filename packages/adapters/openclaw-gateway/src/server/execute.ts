...
} from "@automanager/adapter-utils";
import { asNumber, asString, buildPaperclipEnv, parseObject } from "@automanager/adapter-utils/server-utils";
...
function resolveAutomanagerApiUrlOverride(value: unknown): string | null {
...
function buildPaperclipEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const paperclipApiUrlOverride = resolveAutomanagerApiUrlOverride(ctx.config.paperclipApiUrl);
  const paperclipEnv: Record<string, string> = {
    ...buildPaperclipEnv(ctx.agent),
    AUTOMANAGER_RUN_ID: ctx.runId,
  };

  if (paperclipApiUrlOverride) {
    paperclipEnv.AUTOMANAGER_API_URL = paperclipApiUrlOverride;
  }
  if (wakePayload.taskId) paperclipEnv.AUTOMANAGER_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) paperclipEnv.AUTOMANAGER_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) paperclipEnv.AUTOMANAGER_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) paperclipEnv.AUTOMANAGER_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) paperclipEnv.AUTOMANAGER_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    paperclipEnv.AUTOMANAGER_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const orderedKeys = [
    "AUTOMANAGER_RUN_ID",
    "AUTOMANAGER_AGENT_ID",
    "AUTOMANAGER_COMPANY_ID",
    "AUTOMANAGER_API_URL",
    "AUTOMANAGER_TASK_ID",
    "AUTOMANAGER_WAKE_REASON",
    "AUTOMANAGER_WAKE_COMMENT_ID",
    "AUTOMANAGER_APPROVAL_ID",
    "AUTOMANAGER_APPROVAL_STATUS",
    "AUTOMANAGER_LINKED_ISSUE_IDS",
  ];
  ...
  const apiBaseHint = paperclipEnv.AUTOMANAGER_API_URL ?? "<set AUTOMANAGER_API_URL>";
  ...
  const lines = [
    ...
    ...envLines,
    `AUTOMANAGER_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load AUTOMANAGER_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $AUTOMANAGER_API_KEY on every API call.",
    "- Use X-Paperclip-Run-Id: $AUTOMANAGER_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: AUTOMANAGER_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$AUTOMANAGER_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$AUTOMANAGER_COMPANY_ID/issues?assigneeAgentId=$AUTOMANAGER_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardPaperclipPayload(
  ...
  const templatePaperclip = parseObject(payloadTemplate.automanager);
  ...
  const standardPaperclip: Record<string, unknown> = {
    ...
    apiUrl: paperclipEnv.AUTOMANAGER_API_URL ?? null,
  };
