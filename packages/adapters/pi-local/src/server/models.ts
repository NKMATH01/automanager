...
import type { AdapterModel } from "@automanager/adapter-utils";
import { asString, runChildProcess } from "@automanager/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AUTOMANAGER_PI_COMMAND === "string" &&
    process.env.AUTOMANAGER_PI_COMMAND.trim().length > 0
      ? process.env.AUTOMANAGER_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AUTOMANAGER_", "npm_", "NPM_"] as const;
