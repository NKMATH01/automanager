...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@automanager/adapter-utils";
import {
  asString,
  asNumber,
  asStringArray,
...
const AUTOMANAGER_SESSIONS_DIR = path.join(os.homedir(), ".pi", "paperclips");
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(AUTOMANAGER_SESSIONS_DIR, { recursive: true });
  return AUTOMANAGER_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(AUTOMANAGER_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const desiredPiSkillNames = resolveAutomanagerDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.AUTOMANAGER_WORKSPACE_CWD = workspaceCwd;
