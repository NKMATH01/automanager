...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AUTOMANAGER_OPENCODE_COMMAND === "string" &&
    process.env.AUTOMANAGER_OPENCODE_COMMAND.trim().length > 0
      ? process.env.AUTOMANAGER_OPENCODE_COMMAND.trim()
      : "opencode";
