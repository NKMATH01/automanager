...
import type { RoutineTrigger } from "@automanager/shared";
...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "활성 실행이 진행 중일 때 하나의 후속 실행을 대기열에 유지합니다.",
  always_enqueue: "여러 실행이 쌓이더라도 모든 트리거 발생을 대기열에 추가합니다.",
  skip_if_active: "루틴이 이미 활성 상태일 때 중복 트리거 발생을 무시합니다.",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "루틴 또는 스케줄러가 일시정지된 동안 놓친 스케줄 윈도우를 무시합니다.",
  enqueue_missed_with_cap: "복구 후 놓친 스케줄 윈도우를 제한된 배치로 따라잡습니다.",
};
...
export function RoutineDetail() {
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: "루틴 저장 실패",
        body: error instanceof Error ? error.message : "오토매니저가 루틴을 저장하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "오토매니저가 루틴 실행을 시작하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "오토매니저가 루틴을 업데이트하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "오토매니저가 트리거를 생성하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "오토매니저가 트리거를 업데이트하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: "트리거 삭제 실패",
        body: error instanceof Error ? error.message : "오토매니저가 트리거를 삭제하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "오토매니저가 웹훅 시크릿을 갱신하지 못했습니다.",
        tone: "error",
      });
    },
  });
  ...
  const automationLabel = routine.status === "archived" ? "보관됨" : automationEnabled ? "활성" : "일시정지";
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? "Adding..." : "트리거 추가"}
              </Button>
            </div>
          </div>
