...
/**
 * Filter .automanager.yaml content so it only includes entries whose
...
function generateReadmeFromSelection(
  ...
  if (tasks.length > 0) counts.push(["작업", tasks.length]);
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm automanager company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("자세한 내용은 오토매니저를 참조하세요.");
  lines.push("");
  lines.push("---");
...
export function CompanyExport() {
  ...
  // Recompute .automanager.yaml and README.md content whenever checked files
