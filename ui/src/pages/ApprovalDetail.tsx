...
import type { ApprovalComment } from "@automanager/shared";
...
export function ApprovalDetail() {
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "거부 실패"),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "댓글 작성 실패"),
  });
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">승인 확인됨</p>
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              연결된 이슈는 요청 에이전트가 후속 처리하여 닫을 때까지 열린 상태로 유지됩니다.
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              이 예산 중단을 <Link to="/costs" className="underline underline-offset-2">/costs</Link>의 예산 관리에서 해결하세요.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("이 비승인 에이전트를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.")) return;
