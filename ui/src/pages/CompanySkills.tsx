...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: "URL 관리" };
    case "local":
      return { icon: Folder, label: sourceLabel ?? "폴더", managedLabel: "폴더 관리" };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "오토매니저", managedLabel: "오토매니저 관리" };
    default:
      return { icon: Boxes, label: sourceLabel ?? "카탈로그", managedLabel: "카탈로그 관리" };
  }
}
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                ...
                {detail.sourcePath ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      pushToast({ title: "워크스페이스 경로가 복사되었습니다" });
                    }}
                  >
                    {source.label}
                  </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{detail.editable ? "편집 가능" : "읽기 전용"}</span>
            </div>
          </div>
...
export function CompanySkills() {
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: "가져오기 경고", body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: "스킬 생성됨",
        body: `${skill.name}이(가) 이제 오토매니저 워크스페이스에서 편집 가능합니다.`,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "스킬 생성 실패",
        body: error instanceof Error ? error.message : "스킬 생성 실패.",
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: () => {
      setScanStatusMessage("프로젝트 워크스페이스에서 스킬 스캔 중...");
    },
    onSuccess: async (result) => {
      setScanStatusMessage("스킬 목록 새로고침 중...");
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: "프로젝트 스킬 스캔 실패",
        body: error instanceof Error ? error.message : "프로젝트 워크스페이스 스캔 실패.",
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "저장 실패",
        body: error instanceof Error ? error.message : "스킬 파일 저장 실패.",
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "업데이트 실패",
        body: error instanceof Error ? error.message : "스킬 업데이트 설치 실패.",
      });
    },
  });
