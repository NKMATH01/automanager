...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "실행 중인 작업이 있으면 후속 실행을 하나만 대기열에 유지합니다.",
  always_enqueue: "루틴이 이미 실행 중이더라도 모든 트리거 발생을 대기열에 넣습니다.",
  skip_if_active: "실행 중인 작업이 있으면 새 트리거 발생을 무시합니다.",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "스케줄러 또는 루틴이 일시중지된 동안 놓친 윈도우를 무시합니다.",
...
export function Routines() {
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: "루틴 업데이트 실패",
        body: mutationError instanceof Error ? mutationError.message : "루틴을 업데이트할 수 없습니다.",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: "루틴 실행 실패",
        body: mutationError instanceof Error ? mutationError.message : "루틴 실행을 시작할 수 없습니다.",
        tone: "error",
      });
    },
  });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            감사 가능한 실행 이슈로 구체화되는 반복 작업 정의입니다.
          </p>
        </div>
        ...
        >
              ...
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">캐치업</p>
                      <Select
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              생성 후 스케줄, 웹훅 또는 내부 실행을 위한 트리거 설정으로 바로 이동합니다.
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : "루틴 생성 실패"}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : "루틴 로드 실패"}
          </CardContent>
        </Card>
      ) : null}
      ...
      <div>
        {(routines ?? []).length === 0 ? (
          <div className="py-12">
            <EmptyState
              icon={Repeat}
              message="아직 루틴이 없습니다. 루틴 생성을 사용하여 첫 번째 반복 워크플로우를 정의하세요."
            />
          </div>
        ) : (
