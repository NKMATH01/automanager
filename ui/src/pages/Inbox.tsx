...
import { INBOX_MINE_ISSUE_STATUS_FILTER } from "@automanager/shared";
...
import type { Approval, HeartbeatRun, Issue, JoinRequest } from "@automanager/shared";
import {
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              <span>{timeAgo(approval.updatedAt)} 전 업데이트됨</span>
            </span>
          </span>
        </Link>
...
function JoinRequestInboxRow({
  ...
  const label =
    joinRequest.requestType === "human"
      ? "사용자 가입 요청"
      : `에이전트 가입 요청${joinRequest.agentName ? `: ${joinRequest.agentName}` : ""}`;
  const showUnreadSlot = unreadState !== null;
  const showUnreadDot = unreadState === "visible" || unreadState === "fading";
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{timeAgo(joinRequest.createdAt)} 전 IP {joinRequest.requestIp}에서 요청됨</span>
...
export function Inbox() {
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : "거부 실패");
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={InboxIcon} message="수신함을 보려면 회사를 선택하세요." />;
  }
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="join_requests">가입 요청</SelectItem>
