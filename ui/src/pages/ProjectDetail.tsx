...
import { PROJECT_COLORS, isUuidLike, type BudgetPolicySummary, type ExecutionWorkspace } from "@automanager/shared";
...
function ProjectWorkspacesContent({
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">아직 기본이 아닌 워크스페이스 활동이 없습니다.</p>;
  }
      ...
      >
        <div className="grid gap-4 md:grid-cols-[minmax(0,18rem)_minmax(0,1fr)_auto] md:items-start">
          <div className="min-w-0">
            ...
            {summary.cwd ? (
              <div className="mt-2 flex min-w-0 items-start gap-2 text-xs text-muted-foreground">
                ...
                <CopyText text={summary.cwd} className="shrink-0" copiedLabel="경로가 복사되었습니다">
          ...
          <div className="flex shrink-0 flex-col items-start gap-2 md:items-end">
            ...
            >
              {summary.kind === "project_workspace" ? "워크스페이스 설정" : "워크스페이스 보기"}
            </Link>
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
            ...
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? "닫기 재시도" : "워크스페이스 닫기"}
              </Button>
            ) : null}
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? "프로젝트";
      if (archived) {
        pushToast({ title: `"${name}"이(가) 보관되었습니다`, tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: `"${name}"의 보관이 해제되었습니다`, tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? "프로젝트 보관 실패" : "프로젝트 보관 해제 실패",
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? "프로젝트" },
    ]);
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            ...
            ...(showWorkspacesTab ? [{ value: "workspaces", label: "워크스페이스" }] : []),
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">워크스페이스 로딩 중...</p>
        )
      ) : null}
