...
import type { InstanceSchedulerHeartbeatAgent } from "@automanager/shared";
...
export function InstanceSettings() {
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">스케줄러 하트비트 로드 중...</div>;
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          모든 회사에서 타이머 하트비트가 활성화된 에이전트입니다.
        </p>
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message="현재 기준에 맞는 스케줄러 하트비트가 없습니다."
        />
      ) : (
