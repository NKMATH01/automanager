...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "오토매니저에서 직접 내보낸 .zip 파일을 업로드하세요. Finder, 탐색기 등으로 다시 압축한 파일은 올바르게 가져오지 못할 수 있습니다.";
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: "패키지 읽기 실패",
        body: err instanceof Error ? err.message : "폴더 읽기 실패.",
      });
    }
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: "GitHub 저장소" },
              { key: "local", icon: Upload, label: "로컬 ZIP" },
            ] as const
        ...
        <Field label="대상" hint="이 회사에 가져오거나 새 회사를 생성합니다.">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
