...
import type { BudgetPolicySummary } from "@automanager/shared";
...
export function BudgetPolicyCard({
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? "Saving..." : summary.amount > 0 ? "Update budget" : "예산 설정"}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? "Paused" : summary.status === "warning" ? "Warning" : summary.status === "hard_stop" ? "하드 스톱" : "Healthy"}
          </div>
        </div>
