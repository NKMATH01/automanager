...
type PaperclipSprite = (typeof AUTOMANAGER_SPRITES)[number];
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AUTOMANAGER_SPRITES[(baseCol + baseRow) % AUTOMANAGER_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AUTOMANAGER_SPRITES[Math.floor(Math.random() * AUTOMANAGER_SPRITES.length)]!;
